import manga from '../assets/onlymanga.png'
import healthshark from '../assets/healthshark.jpg'

const Project = () => {
    return (
        <section className='relative top-60 mx-auto' id='project'>
            <div className='w-[95%] max-w-[750px] h-auto'>
                <h1 className='text-3xl font-semibold text-center mb-8'>Project</h1>

                <div className='flex flex-col items-center gap-8 w-full'>
                <a>
                    <div className='flex w-full rounded-2xl bg-[#e0dcdc] hover:bg-white transition-all duration-200 hover:scale-105 cursor-pointer
                                    outline-none border-none'>
                        <img src={manga} className='w-1/3 object-cover rounded-l-2xl'/>
                        <div className='w-2/3 text-black p-5 rounded-lg flex flex-col justify-between'>
                            <div>
                                <h3 className='text-2xl font-semibold mb-2'>Only Managa - May 2025</h3>
                                <p className='text-[15px]'>Developed a Full-stack E-Commerce Web Application to sells manga with an admin dashboard for managing the website</p>
                            </div>
                            <div className='flex flex-wrap gap-2 items-center'>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    React.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Javascript</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Node.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Express.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    MongoDB</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Restful API</span>
                            </div>
                        </div>
                    </div>
                </a>

                <a>
                    <div className='flex w-full rounded-2xl bg-[#e0dcdc] hover:bg-white transition-all duration-200 hover:scale-105 cursor-pointer
                                    outline-none border-none'>
                        <img src={healthshark} className='w-1/3 object-cover rounded-l-2xl'/>
                        <div className='w-2/3 text-black p-5 rounded-lg flex flex-col justify-between'>
                            <div>
                                <h3 className='text-2xl font-semibold mb-2'>Health Shark - March 2025</h3>
                                <p className='text-[15px]'>Developed an AI Chatbot to treat emotional disorders with animated</p>
                            </div>
                            <div className='flex flex-wrap gap-2 items-center'>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    React.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Javascript</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Node.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Express.js</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    MongoDB</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Python</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    FastAPI</span>
                                <span className='text-[12px] font-semibold text-blue-800 bg-blue-100 rounded-full px-2.5 py-0.5'>
                                    Transformer</span>
                            </div>
                        </div>
                    </div>
                </a>
                </div>
            </div>  
        </section>
    )
}

export default Project